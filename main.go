...
func main() {
	...
	if *specPath == "" || *baseURL == "" || *token == "" {
		fmt.Println("Usage: --spec spec.json --base-url https://api.example.com --token <jwt> --outdir reports")
		os.Exit(1)
	}
	...
	results := tester.RunTests(endpoints, *token)
